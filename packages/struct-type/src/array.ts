...
import type { InitType, RuntimeType, Type } from "./index.js";
import { getValueFromView, NumberType, putValueInView } from "./numbers.js";
import { bigint64, biguint64 } from "./primitives.js";
import { FixedLengthArrayTypeInfo, PrimitiveTypeInfo, ArrayTypeInfo } from "./typeInfo.js";
...
class ElementCache<TType extends Type> {
    #cache: (RuntimeType<TType> | undefined)[] | undefined;
    ...
    get hasValues() { return this.#cache !== undefined; }
    get values() { return this.#cache ??= Array(this.fixedLength); }
}
...
class ElementCacheView<TType extends Type> {
    ...
    get(index: number) {
        ...
        return this.cache.values[index + this.#offset];
    }

    set(index: number, value: RuntimeType<TType>) {
        if (index < 0) return;
        if (index >= this.#length) return;
        this.cache.values[index + this.#offset] = value;
    }

    slice(start: number, end: number) {
        return this.cache.values.slice(start + this.#offset, end + this.#offset);
    }
}
...
abstract class TypedArray<TType extends Type> extends IntegerIndexedObject<RuntimeType<TType>> {
    ...
    constructor(...args: TypedArrayConstructorOverloads<TType>) {
        ...
        if (isTypedArrayConstructorArrayBufferLikeOverload(args)) {
            ...
            if (byteOffset > buffer.byteLength - byteLength) throw new RangeError("Out of range: byteOffset");
            if (byteOffset % type.alignment) throw new RangeError(`Not aligned: byteOffset must be a multiple of ${type.alignment}`);
