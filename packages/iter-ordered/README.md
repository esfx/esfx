...
class BooksOrderedIterable implements OrderedIterable<Book> {
    ...
    static orderBy<K>(books: Iterable<Book>, keySelector: (element: T) => K, keyComparer: Comparison<K> | Comparer<K>, descending: boolean): OrderedIterable<T> {
        if (typeof keyComparer === "function") keyComparer = Comparer.create(keyComparer);
        return new BooksOrderedIterable(books, [{ keySelecor, keyComparer, descending }]);
    }
    ...
    [OrderedIterable.thenBy]<K>(keySelector: (element: T) => K, keyComparer: Comparison<K> | Comparer<K>, descending: boolean): OrderedIterable<T> {
        if (typeof keyComparer === "function") keyComparer = Comparer.create(keyComparer);
        return new BooksOrderdIterable(this._books, [...this._orderBy, { keySelecor, keyComparer, descending }]);
    }
...
You can read more about the API [here](https://esfx.js.org/esfx/api/iter-ordered.html).
