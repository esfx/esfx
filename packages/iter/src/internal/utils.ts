...
import { isHierarchyIterable } from './guards';
import { startsWith, empty } from '../fn';
import { Hierarchical, HierarchyProvider, HierarchyIterable } from '../hierarchy';
...
export function flowHierarchy<T, TIter extends Iterable<T>>(to: TIter, fromLeft: Iterable<T>, fromRight?: Iterable<T>): TIter {
    const leftProvider = isHierarchyIterable(fromLeft) ? fromLeft[Hierarchical.hierarchy]() : undefined;
    const rightProvider = isHierarchyIterable(fromRight) ? fromRight[Hierarchical.hierarchy]() : undefined;
